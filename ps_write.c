...
#pragma omp parallel for private(i,tid,l,kmineff,keff) shared(NGRID,K,kmin,kx,ky,kz,Mono,Quadru,P0,P2,nmodes,Deltak,Nk,bintype_sw)
        for(i=0;i<NGRID;i++)
        {
                ...
                {
                        l=l+1;
if(bintype_sw==0){keff=pow(kx[i]*kx[i]+ky[i]*ky[i]+kz[i]*kz[i],0.5);kmineff=(l+1)*Deltak+kmin;}
if(bintype_sw==1){keff=log10(pow(kx[i]*kx[i]+ky[i]*ky[i]+kz[i]*kz[i],0.5)); kmineff=(l+1)*Deltak+log10(kmin);}
...
#pragma  omp parallel for private(index2,l2,i,j,k,l,tid,i2,k2,j2,keff,kmineff) shared(ngrid,ngridtot,kx,Deltak,Nk,K,deltak_re0,deltak_im0,deltak_re2,deltak_im2,nmodes,Mono,Quadru,Hexadeca,N_interlacing,kmin,bintype_sw)
        for(l2=0;l2<ngridtot;l2++)
        {
                ...
                {
                        l=l+1;
if(bintype_sw==0){keff=pow(kx[i]*kx[i]+kx[j]*kx[j]+kx[k]*kx[k],0.5);kmineff=(l+1)*Deltak+kmin;}
if(bintype_sw==1){keff=log10(pow(kx[i]*kx[i]+kx[j]*kx[j]+kx[k]*kx[k],0.5)); kmineff=(l+1)*Deltak+log10(kmin);}
...
if(bintype_sw==0){keff=pow(kx[i]*kx[i]+kx[j]*kx[j]+kx[k]*kx[k],0.5);kmineff=(l+1)*Deltak+kmin;}
if(bintype_sw==1){keff=log10(pow(kx[i]*kx[i]+kx[j]*kx[j]+kx[k]*kx[k],0.5)); kmineff=(l+1)*Deltak+log(kmin);}
